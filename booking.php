...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$room_id || !$request_date || !$payment_method) {
        $error = 'Заполните все поля заявки.';
    } else {
        $stmt = $mysqli->prepare("INSERT INTO bookings (user_id, room_id, request_date, payment_method) VALUES (?, ?, ?, ?)");
        $stmt->bind_param('iiss', $_SESSION['user_id'], $room_id, $request_date, $payment_method);
        $stmt->execute();
