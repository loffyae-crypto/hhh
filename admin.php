...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = $_POST['status'];
    if (in_array($status, ['Новая','Заявка подтверждена','Заявка завершена'])) {
...
<table>
<tr><th>Пользователь</th><th>Контакты</th><th>Объект</th><th>Дата</th><th>Оплата</th><th>Статус</th></tr>
<?php while ($b = $bookings->fetch_assoc()): ?>
<tr>
    <td><?= htmlspecialchars($b['full_name']) ?></td>
    <td><?= htmlspecialchars($b['phone']) ?><br><?= htmlspecialchars($b['email']) ?></td>
    <td><?= htmlspecialchars($b['room_name']) ?></td>
    <td><?= date('d.m.Y H:i', strtotime($b['request_date'])) ?></td>
    <td><?= htmlspecialchars($b['payment_method']) ?></td>
    <td>
        <form method="post" class="inline-form">
            <input type="hidden" name="booking_id" value="<?= $b['id'] ?>">
            <select name="status">
                <?php foreach (['Новая','Заявка подтверждена','Заявка завершена'] as $s): ?>
